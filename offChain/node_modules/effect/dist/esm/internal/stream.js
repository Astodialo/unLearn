...
export const toReadableStreamRuntime = /*#__PURE__*/dual(args => isStream(args[0]), (self, runtime, options) => {
  ...
  return new ReadableStream({
    start(controller) {
      ...
      const pullChunk = pipe(toPull(self), Scope.extend(scope), runSync, Effect.flatMap(chunk => Chunk.isEmpty(chunk) ? pullChunk : Effect.succeed(chunk)));
      pull = pipe(pullChunk, Effect.tap(chunk => Effect.sync(() => {
        Chunk.map(chunk, a => {
