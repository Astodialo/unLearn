...
__export(src_exports, {
  ...
  generateEmulatorAccountFrommPrivateKey: () => generateEmulatorAccountFrommPrivateKey
});
            ...
            scriptRef: r.reference_script_hash ? await (async () => {
              ...
              switch (type) {
                case "timelock":
                  ...
                  return (0, import_utils.scriptFromNative)(native);
                case "plutusV1":
                  return {
                    type: "PlutusV1",
                    script: (0, import_utils.applyDoubleCborEncoding)(script)
                  };
                case "plutusV2":
                  return {
                    type: "PlutusV2",
...
var toOgmiosUTxOs = (utxos) => {
  ...
  const toOgmiosAssets = (assets) => {
    ...
    Object.entries(assets).forEach(([unit, amount]) => {
      ...
      const { policyId, assetName } = (0, import_utils2.fromUnit)(unit);
...
// src/kupmios.ts
var KupmiosError = class extends import_effect2.Data.TaggedError("KupmiosError") {
  get message() {
...
var Kupmios = class {
  ...
  async getProtocolParameters() {
    ...
    const program = fetchOgmiosParse(this.ogmiosUrl, data, schema).pipe(
      import_effect2.Effect.flatMap(
        (response) => "error" in response ? import_effect2.Effect.fail(response) : import_effect2.Effect.succeed(response.result)
      ),
      import_effect2.Effect.timeout(1e4),
      import_effect2.Effect.catchAll((cause) => new KupmiosError({ cause }))
    );
    const result = await import_effect2.Effect.runPromise(program);
  ...
  async getUtxos(addressOrCredential) {
    ...
    const program = fetchKupoParse(pattern, schema).pipe(
      import_effect2.Effect.flatMap((u) => kupmiosUtxosToUtxos(this.kupoUrl, u)),
      import_effect2.Effect.timeout(1e4),
      import_effect2.Effect.catchAll((cause) => new KupmiosError({ cause }))
    );
    const utxos = await import_effect2.Effect.runPromise(program);
    return utxos;
  }
  async getUtxosWithUnit(addressOrCredential, unit) {
    const isAddress = typeof addressOrCredential === "string";
  ...
  async getUtxosByOutRef(outRefs) {
    ...
    const schema = S3.Array(UTxOSchema);
    const program = import_effect2.Effect.forEach(
      queryHashes,
    ...
    const utxos = await import_effect2.Effect.runPromise(program);
    return import_effect2.Array.flatten(utxos).filter(
      (utxo) => outRefs.some(
        (outRef) => utxo.txHash === outRef.txHash && utxo.outputIndex === outRef.outputIndex
      )
    );
  ...
  async getDatum(datumHash) {
    ...
    const program = fetchKupoParse(pattern, schema).pipe(
      import_effect2.Effect.timeout(1e4),
      import_effect2.Effect.flatMap(import_effect2.Effect.fromNullable),
      import_effect2.Effect.catchAll((cause) => new KupmiosError({ cause }))
    );
    const result = await import_effect2.Effect.runPromise(program);
    return result.datum;
  ...
  async awaitTx(txHash, checkInterval = 2e4) {
    ...
    const program = fetchKupoParse(pattern, schema).pipe(
      import_effect2.Effect.repeat({
        schedule: import_effect2.Schedule.exponential(checkInterval),
        until: (result) => result.length > 0
      }),
      import_effect2.Effect.timeout(16e4),
      import_effect2.Effect.catchAll((cause) => new KupmiosError({ cause })),
      import_effect2.Effect.as(true)
    );
    return await import_effect2.Effect.runPromise(program);
  }
  async submitTx(cbor) {
    const data = {
      jsonrpc: "2.0",
    ...
    const program = fetchOgmiosParse(this.ogmiosUrl, data, schema).pipe(
      import_effect2.Effect.flatMap(
        (response) => "error" in response ? import_effect2.Effect.fail(response) : import_effect2.Effect.succeed(response)
      ),
      import_effect2.Effect.timeout(1e4),
      import_effect2.Effect.catchAll((cause) => new KupmiosError({ cause }))
    );
...
var getDatumEffect = (kupoUrl, datum_type, datum_hash) => import_effect2.Effect.gen(function* () {
  if (datum_type === "inline" && datum_hash) {
    const pattern = `${kupoUrl}/datums/${datum_hash}`;
    const schema = DatumSchema;
    return yield* fetchKupoParse(pattern, schema).pipe(
      import_effect2.Effect.flatMap(import_effect2.Effect.fromNullable),
      import_effect2.Effect.map((result) => result.datum),
      import_effect2.Effect.retry(
        import_effect2.Schedule.compose(import_effect2.Schedule.exponential(50), import_effect2.Schedule.recurs(5))
      ),
      import_effect2.Effect.timeout(5e3)
    );
  } else return void 0;
});
var getScriptEffect = (kupoUrl, script_hash) => import_effect2.Effect.gen(function* () {
  if (script_hash) {
    const pattern = `${kupoUrl}/scripts/${script_hash}`;
    const schema = ScriptSchema;
    return yield* (0, import_effect2.pipe)(
      ...
      import_effect2.Effect.timeout(5e3),
      import_effect2.Effect.map(({ language, script }) => {
        switch (language) {
          case "native":
            return {
              type: "Native",
          ...
          case "plutus:v1":
            return {
              type: "PlutusV1",
              script: (0, import_utils3.applyDoubleCborEncoding)(script)
            };
          case "plutus:v2":
            return {
              type: "PlutusV2",
...
var fetchOgmiosParse = (url, data, schema) => (0, import_effect2.pipe)(
  import_platform.HttpClientRequest.post(url),
  import_platform.HttpClientRequest.jsonBody(data),
  import_effect2.Effect.flatMap(import_platform.HttpClient.fetch),
  import_platform.HttpClientResponse.json,
  import_effect2.Effect.flatMap(S3.decodeUnknown(schema)),
  import_effect2.Effect.catchTag(
    "ParseError",
    (e) => import_effect2.Effect.fail(import_schema.ArrayFormatter.formatErrorSync(e))
  )
  // Effect.scoped
);
var fetchKupoParse = (url, schema) => (0, import_effect2.pipe)(
  import_platform.HttpClientRequest.get(url),
  import_platform.HttpClient.fetchOk,
  import_platform.HttpClientResponse.json,
  import_effect2.Effect.flatMap(S3.decodeUnknown(schema, { onExcessProperty: "error" })),
  import_effect2.Effect.catchTag(
    "ParseError",
    (e) => import_effect2.Effect.fail(import_schema.ArrayFormatter.formatErrorSync(e))
  )
...
var kupmiosUtxosToUtxos = (kupoURL, utxos) => import_effect2.Effect.forEach(
  utxos,
  (utxo) => {
...
function generateEmulatorAccount(assets) {
  const seedPhrase = (0, import_utils4.generateSeedPhrase)();
  return {
    seedPhrase,
...
var Emulator = class {
  ...
  protocolParameters;
  datumTable = {};
  constructor(accounts, protocolParameters = import_utils4.PROTOCOL_PARAMETERS_DEFAULT) {
  ...
  getUtxos(addressOrCredential) {
    const utxos = Object.values(this.ledger).flatMap(({ utxo }) => {
      if (typeof addressOrCredential === "string") {
        ...
        const { paymentCredential } = (0, import_utils5.getAddressDetails)(utxo.address);
...
It does not exist or was already spent.`
      ...
      const { paymentCredential } = (0, import_utils5.getAddressDetails)(entry.utxo.address);
    ...
    for (let index = 0; index < (body.withdrawals()?.keys().len() || 0); index++) {
      ...
      const { stakeCredential } = (0, import_utils5.getAddressDetails)(rewardAddress);
    ...
    const outputs = (() => {
      ...
      for (let i = 0; i < body.outputs().len(); i++) {
        ...
        const utxo = (0, import_utils5.coreToUtxo)(unspentOutput);
    ...
    if (arrLegacyRedeemer) {
      for (let i = 0; i < arrLegacyRedeemer.len(); i++) {
        ...
        evalRedeemers.push({
          ...
          redeemer_tag: (0, import_utils4.fromCMLRedeemerTag)(legacyRedeemer.tag())
        });
      }
      return evalRedeemers;
    }
      ...
      for (let i = 0; i < keys.len(); i++) {
        ...
        if (redeemerVal) {
          evalRedeemers.push({
            ...
            redeemer_tag: (0, import_utils4.fromCMLRedeemerTag)(key.tag())
          });
        }
      }
      return evalRedeemers;
...
var Maestro = class {
  ...
  async evaluateTx(tx, additionalUTxOs) {
    const additionalMaestroUTxOs = (additionalUTxOs || []).map((utxo) => ({
      tx_hash: utxo.txHash,
      index: utxo.outputIndex,
      txout_cbor: (0, import_utils6.utxoToCore)(utxo).output().to_cbor_hex()
    }));
    const payload = {
      cbor: tx,
...
var toScriptRef = (reference_script) => {
  if (reference_script && reference_script.bytes) {
    switch (reference_script.type) {
      case "native":
        return (0, import_utils6.scriptFromNative)(reference_script.json);
      case "plutusv1":
        return {
          type: "PlutusV1",
          script: (0, import_utils6.applyDoubleCborEncoding)(reference_script.bytes)
        };
      case "plutusv2":
        return {
          type: "PlutusV2",
...
// src/koios.ts
var S5 = __toESM(require("@effect/schema/Schema"), 1);
...
var S4 = __toESM(require("@effect/schema/Schema"), 1);
...
var postWithSchemaValidation = (url, data, schema) => (0, import_effect3.pipe)(
  import_platform2.HttpClientRequest.post(url),
  import_platform2.HttpClientRequest.jsonBody(data),
  import_effect3.Effect.flatMap(import_platform2.HttpClient.fetchOk),
  import_platform2.HttpClientResponse.json,
  import_effect3.Effect.flatMap(S4.decodeUnknown(schema))
  // Effect.scoped
);
var getWithSchemaValidation = (url, schema) => (0, import_effect3.pipe)(
  import_platform2.HttpClientRequest.get(url),
  import_platform2.HttpClient.fetchOk,
  import_platform2.HttpClientResponse.json,
  import_effect3.Effect.flatMap(S4.decodeUnknown(schema))
);
...
var getUtxosEffect = (baseUrl, addressOrCredential) => {
  ...
  const result = (0, import_effect3.pipe)(
    import_effect3.Effect.if(typeof addressOrCredential === "string", {
      onFalse: () => import_effect3.Effect.fail("Credential Type is not supported in Koios yet."),
      onTrue: () => postWithSchemaValidation(url, body, schema)
    }),
    import_effect3.Effect.map(
...
var KoiosError = class extends import_effect4.Data.TaggedError("KoiosError") {
  get message() {
    return `${this.cause}`;
  }
};
var Koios = class {
  ...
  async getProtocolParameters() {
    ...
    const [result] = await (0, import_effect4.pipe)(
      getWithSchemaValidation(url, schema),
      import_effect4.Effect.timeout(1e4),
      import_effect4.Effect.catchAllCause((cause) => new KoiosError({ cause })),
      import_effect4.Effect.runPromise
    );
    return {
  ...
  async getUtxoByUnit(unit) {
    let { policyId, assetName } = (0, import_utils8.fromUnit)(unit);
    ...
    const result = await (0, import_effect4.pipe)(
      ...
      import_effect4.Effect.flatMap(
        (adresses) => adresses.length === 0 ? import_effect4.Effect.fail("Unit not found") : import_effect4.Effect.succeed(adresses)
      ),
      import_effect4.Effect.flatMap(
        (adresses) => adresses.length > 1 ? import_effect4.Effect.fail("Unit needs to be an NFT or only held by one address.") : import_effect4.Effect.succeed(adresses[0])
      ),
      import_effect4.Effect.flatMap(
      ...
      import_effect4.Effect.flatMap(
        (utxos) => utxos.length > 1 ? import_effect4.Effect.fail("Unit needs to be an NFT or only held by one address.") : import_effect4.Effect.succeed(utxos[0])
      ),
      import_effect4.Effect.timeout(1e4),
      import_effect4.Effect.catchAllCause((cause) => new KoiosError({ cause })),
      import_effect4.Effect.runPromise
  ...
  async getDelegation(rewardAddress) {
    ...
    const result = await (0, import_effect4.pipe)(
      ...
      import_effect4.Effect.flatMap(
        (result2) => result2.length === 0 ? import_effect4.Effect.fail("No Delegation Found by Reward Address") : import_effect4.Effect.succeed(result2[0])
      ),
      import_effect4.Effect.timeout(1e4),
      import_effect4.Effect.catchAllCause((cause) => new KoiosError({ cause })),
      import_effect4.Effect.runPromise
  ...
  async getDatum(datumHash) {
    ...
    const result = await (0, import_effect4.pipe)(
      ...
      import_effect4.Effect.flatMap(
        (result2) => result2.length === 0 ? import_effect4.Effect.fail("No Datum Found by Datum Hash") : import_effect4.Effect.succeed(result2[0])
      ),
      import_effect4.Effect.timeout(1e4),
      import_effect4.Effect.catchAllCause((cause) => new KoiosError({ cause })),
      import_effect4.Effect.runPromise
  ...
  async awaitTx(txHash, checkInterval = 2e4) {
    ...
    const result = await (0, import_effect4.pipe)(
      postWithSchemaValidation(url, body, schema),
      import_effect4.Effect.repeat({
        schedule: import_effect4.Schedule.exponential(checkInterval),
        until: (result2) => result2.length > 0
      }),
      import_effect4.Effect.timeout(16e4),
      import_effect4.Effect.orDie,
      import_effect4.Effect.as(true),
      import_effect4.Effect.runPromise
    );
    return result;
  }
  ...
  async evaluateTx(tx, additionalUTxOs) {
    ...
    const result = await (0, import_effect4.pipe)(
      postWithSchemaValidation(url, body, schema),
      import_effect4.Effect.flatMap(
        (response) => "error" in response ? import_effect4.Effect.fail(response) : import_effect4.Effect.succeed(response.result)
      ),
      import_effect4.Effect.timeout(1e4),
      import_effect4.Effect.catchAllCause((cause) => new KoiosError({ cause })),
      import_effect4.Effect.runPromise
