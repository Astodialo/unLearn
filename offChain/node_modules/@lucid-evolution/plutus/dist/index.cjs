...
var TypeBox = __toESM(require("@sinclair/typebox"), 1);
...
__reExport(data_exports, require("@sinclair/typebox"));
var Constr = class {
  index;
  fields;
  ...
  function serialize(data2) {
    try {
      if (typeof data2 === "bigint") {
        ...
        return CML.PlutusData.new_bytes((0, import_core_utils.fromHex)(data2));
...
function from(raw, type) {
  function deserialize(data2) {
    if (data2.kind() === 0) {
      ...
      return (0, import_core_utils.toHex)(data2.as_bytes());
    }
    throw new Error("Unsupported type");
  }
...
function fromJson(json) {
  function toData(json2) {
    if (typeof json2 === "string") {
      return json2.startsWith("0x") ? (0, import_core_utils.toHex)((0, import_core_utils.fromHex)(json2.slice(2))) : (0, import_core_utils.fromText)(json2);
    }
    ...
    if (typeof data === "string") {
      try {
        return new TextDecoder(void 0, { fatal: true }).decode(
          (0, import_core_utils.fromHex)(data)
        );
      } catch (_) {
        return "0x" + (0, import_core_utils.toHex)((0, import_core_utils.fromHex)(data));
      }
    }
        ...
        case "object": {
          ...
          return new Constr(
            ...
            args instanceof Array ? args.map(
              (item, index) => castTo(item, enumEntry.fields[index])
    ...
    case "list": {
      ...
      if (shape.items instanceof Array) {
        const fields = struct.map(
          (item, index) => castTo(item, shape.items[index])
        );
...
// src/index.ts
__reExport(src_exports, data_exports, module.exports);
